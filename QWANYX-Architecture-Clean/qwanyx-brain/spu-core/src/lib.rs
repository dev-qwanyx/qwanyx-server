...
pub enum Instruction {
    ...
    // SPU 1.0 additions
    Return { value: String },
    Throw { error_type: String, message: String },
    Expr { expression: String, target: String },
    Foreach { item: String, collection: String, body: Vec<Instruction> },
    Break,
    Continue,
    Async { object: String, method: String, args: Data, handle: String },
    Await { handle: String, target: String },
    Function { name: String, params: Vec<String>, body: Vec<Instruction> },
    CallFn { name: String, args: Vec<Data>, target: String },
    Len { collection: String, target: String },
    Parallel { tasks: Vec<Vec<Instruction>>, target: String },
    Race { tasks: Vec<Vec<Instruction>>, target: String }
}
        ...
        match instruction {
            ...
            Instruction::GetHealth { object, target } => {
                ...
                if let Some(coprocessor) = instances.get(object) {
                    ...
                    let json_health = serde_json::to_value(health)
                        .map_err(|e| SPUError::SerializationError(e.to_string()))?;
                    
                    let mut memory = self.memory.write().await;
                    memory.insert(target.clone(), Data::from_json(json_health));
                } else {
                    return Err(SPUError::ObjectNotFound(object.clone()));
                }
            }
